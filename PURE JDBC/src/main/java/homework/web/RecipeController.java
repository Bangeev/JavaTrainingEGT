...
public class RecipeController {
    ...
    @GetMapping
    public Collection<Recipe> getAllRecipes(@RequestParam(name = "tags", required = false) String tag,
                                            @RequestParam(name = "name", required = false) String name) {
        if (tag!= null && !tag.trim().isEmpty()) {

            return recipeService.getAllRecipesByTags(tag);
    ...
    @PutMapping("/{id}")
    Optional<Recipe> updateRecipe(@PathVariable Long id, @Valid @RequestBody Recipe recipe, Errors errors) {
        if (errors.hasErrors()) {
            throw new InvalidDataException("Invalid user data", getAllErrors(errors));
        }
